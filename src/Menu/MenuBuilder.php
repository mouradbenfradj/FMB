...
class MenuBuilder
{
    ...
    /* $menu->addChild('Prod à faire') 
            ...
            ->setLinkAttribute('id', 'topnav-dashboard')
            ->setLinkAttribute('data-toggle', 'dropdown')
            ->setLinkAttribute('aria-haspopup', 'true')
            ->setLinkAttribute('aria-expanded', 'false')
            ->setLinkAttribute('role', 'button')
            ->setLabel('<i class="fe-airplay mr-1"></i> Dashboards <div class="arrow-down"></div>')
            ->setExtra('safe_label',true)
            ->setChildrenAttribute('class', 'dropdown-menu')
            ->setChildrenAttribute('aria-labelledbyaria-labelledby', 'topnav-dashboard');
        $menu['Prod à faire']->addChild('Dashboard')
            ->setUri("#")
            ->setLinkAttribute('class','dropdown-item');
        $menu['Prod à faire']->addChild('Dashboard 2')
            ->setUri("#")
            ->setLinkAttribute('class','dropdown-item');
        $menu['Prod à faire']->addChild('Dashboard 3')
            ->setUri("#")
            ->setLinkAttribute('class','dropdown-item');
    ...
    public function createNewMainMenu(RequestStack $requestStack)
    {
        ...
        if ($this->_security->isGranted('ROLE_SUPER_ADMIN')) {
            ...
            $menu['Prod à faire']->addChild('MAE Cordes');
            $menu['Prod à faire']->addChild('MAE Poches');
            $menu['Prod à faire']->addChild('MAE Assemblages');
            $menu['Prod à faire']->addChild('Passage Chaussettes');
            $menu['Prod à faire']->addChild('Retrait Transfert');
            $menu['Prod à faire']->addChild('Retrait AW Lanternes');
            $menu['Prod à faire']->addChild('Retrait AW Cordes');
            $menu['Prod à faire']->addChild('Traitement Comercial');
            $menu['Prod à faire']['Préparation']->addChild('Préparer Lanternes', array('route' => 'app_preparationlanterne'));
            $menu['Prod à faire']['Préparation']->addChild('Préparer Cordes', array('route' => 'app_preparationcorde'));
            $menu['Prod à faire']['Préparation']->addChild('Préparer Poches', array('route' => 'app_preparationpoche'));
            $menu['Prod à faire']['Assemblage']->addChild('Poches à Cordes', array('route' => 'app_assemblage'));
            $menu['Outils de gestion']['Prévisions des sorties']->addChild('Tableaux', array('route' => 'previsionFuture'));
